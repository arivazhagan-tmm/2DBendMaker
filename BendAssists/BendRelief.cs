...
public class BendRelief {
   ...
   #region Methods -------------------------------------------------
   /// <summary>Adds bend relief to the part</summary>
   /// Takes the part and finds bend relief points
   /// creates new lines based on the bend relief points
   /// returns the bend processed processedPart based on the new list of lines
   public bool ApplyBendRelief (Part part, out BendProcessedPart processedPart) {
      processedPart = new BendProcessedPart ();
      mPLines.Clear ();
      foreach (var line in part.PLines) mPLines.Add (line);
      var hLines = mPLines.Where (IsHorizontal).ToList ();
      var vLines = mPLines.Where (IsVertical).ToList ();
      if (part.BendLines.Count != 0)
         foreach (var bLine in part.BendLines) {
            ...
            // Check if the part has a stepcut
            ...
            AddBendReliefPoints (bLine, iLine, nLine, iPoint, niPoint, part.Vertices.Centroid (), part.Thickness);
         }
      else return false;
      processedPart = new BendProcessedPart (GetOrderedLines (mPLines), part.BendLines, true);
      return true;
   }
   #endregion
