...
public class BendRelief {
   ...
   public BendProcessedPart ApplyBendRelief (Part profile) {
      ...
      if (profile.BendLines.Count () != 0) {
         foreach (var bLine in profile.BendLines) {
            ...
            AddBendReliefPoints (bLine, centroid, NIPoint, NPCurve, ICurve, offset, IPoint, profile.Thickness);
         }
      }
   ...
   void AddBendReliefPoints (BendLine bLine, BPoint centre, BPoint NIPoint, PLine NPCurve, PLine ICurve, double offset, BPoint IPoint, double thickness) {
      double brHeight = BendUtils.GetBendAllowance (90, 0.38, thickness, 2) / 2, brWidth = thickness / 2;
      ...
      List<BPoint> points = bLine.Orientation is EBLOrientation.Horizontal ? ((IPoint.X > centre.X && IPoint.Y > centre.Y)
      ...
      if (mHasCornerNotch) points.Remove (p5);
      for (int i = 0; i <= points.Count - 2; i++)
         mPLines.Add (new PLine (ECurve.Line, -1, points[i], points[i + 1]));
   ...
   public List<PLine> GetOrderedLines (List<PLine> pLines) {
      ...
      while (pLines.Count > 0) {
         PLine lastLine = orderedPLines.Last ();
         BPoint lastEndPoint = lastLine.EndPoint;
         var nxtLine = pLines.FirstOrDefault (line => line.StartPoint.X == lastEndPoint.X && line.StartPoint.Y == lastEndPoint.Y);
